...
import { Settings as SettingsIcon, Mic, Volume2, Save, RefreshCw, Database, Tag, Calendar, Mail, CheckCircle2, XCircle, Target, X, Plus, Bot, Zap, ShieldCheck, ShieldOff, ShieldQuestion, MessageCircle, ExternalLink, Copy, Loader2, Send, Landmark, CreditCard, DollarSign, Building2 } from "lucide-react";
...
export default function SettingsPage() {
  ...
  // Plaid / Financial integration status
  ...
  const { data: plaidInstitutions = [], refetch: refetchInstitutions } = useQuery<PlaidInstitution[]>({
    ...
    enabled: plaidStatus?.configured && plaidStatus?.enabled,
  });

  const { data: financialAccounts = [], refetch: refetchAccounts } = useQuery<FinancialAccount[]>({
  ...
  const plaidLinkMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/plaid/link-token");
      return response.json();
    },
    onSuccess: async (data) => {
      if (data.linkToken && typeof window !== 'undefined') {
    ...
    onError: () => {
      toast({ title: "Failed to start connection flow", variant: "destructive" });
      setIsConnectingPlaid(false);
    },
  });

  const plaidDisconnectMutation = useMutation({
    mutationFn: (itemId: string) => apiRequest("DELETE", `/api/plaid/institutions/${itemId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/plaid/institutions"] });
      queryClient.invalidateQueries({ queryKey: ["/api/plaid/accounts"] });
  ...
  const plaidSyncMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({ 
        ...
        description: `Added ${data.added} new transactions` 
      });
    },
    onError: () => {
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <Card className="glass-card border-white/20">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Connect your bank accounts to track spending and include financial insights in your morning briefings.
              </p>
              ...
              {isPlaidLoading ? (
                ...
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
                  <p className="text-sm text-amber-500">
                    Plaid is not configured. Add PLAID_CLIENT_ID, PLAID_SECRET, and PLAID_ENV to secrets.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {plaidInstitutions.length > 0 ? (
                    <div className="space-y-3">
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-sm font-medium">{inst.institutionName || 'Bank Account'}</p>
                                <p className="text-xs text-muted-foreground">
                                  {inst.lastSyncedAt 
                                    ? `Last synced: ${new Date(inst.lastSyncedAt).toLocaleDateString()}`
                                    : 'Never synced'
                                  }
                                </p>
                              </div>
                            ...
                            <div className="flex items-center gap-2">
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => plaidSyncMutation.mutate(inst.id)}
                                disabled={plaidSyncMutation.isPending}
                                data-testid={`button-sync-${inst.id}`}
                              >
                                {plaidSyncMutation.isPending ? (
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => plaidDisconnectMutation.mutate(inst.id)}
                                disabled={plaidDisconnectMutation.isPending}
                                data-testid={`button-disconnect-${inst.id}`}
                              >
                          ...
                          {financialAccounts.filter(a => !a.isHidden).length > 0 && (
                            <div className="mt-3 space-y-2 pl-8">
                              {financialAccounts.slice(0, 5).map((account) => (
                      ...
                      <Button
                        variant="outline"
                        className="w-full"
                        onClick={() => plaidLinkMutation.mutate()}
                        disabled={plaidLinkMutation.isPending || isConnectingPlaid}
                        data-testid="button-plaid-add-another"
                      >
                        {(plaidLinkMutation.isPending || isConnectingPlaid) ? (
                          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                        ) : (
                  ...
                  {plaidInstitutions.length > 0 && (
                    <>
                      <div className="flex items-center justify-between pt-2">
                        <div className="space-y-0.5">
                          <Label className="text-base">Include in Briefings</Label>
                          <p className="text-xs text-muted-foreground">
                            Add spending insights to morning briefings
                          </p>
                        </div>
                        <Switch
                          data-testid="switch-plaid-briefings"
                          checked={settings.plaidIncludeInBriefings !== false}
                          onCheckedChange={(checked) => 
                            setSettings(prev => ({ ...prev, plaidIncludeInBriefings: checked }))
                          }
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="space-y-0.5">
                          <Label className="text-base">Enable Financial Tracking</Label>
                          <p className="text-xs text-muted-foreground">
                            Master toggle for financial features
                          </p>
                        </div>
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            setSettings(prev => ({ ...prev, plaidEnabled: checked }))
                          }
                        />
                      </div>
                    </>
