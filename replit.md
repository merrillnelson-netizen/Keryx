...
- **Ideas & Workspace**: Versatile workspace (/ideas) supporting multiple types: Ideas (full AI brainstorming with stages), Notes (quick text capture), Lists (checkable items like grocery/packing lists), and Documents (structured content). Type-aware AI assistance provides contextual help - list item suggestions, note summarization, writing feedback for documents, and brainstorming for ideas. Ideas progress through stages: Spark → Exploring → Planning → In Progress → Completed/Dropped. Features include AI chat, task breakdown, and type-specific UIs.
